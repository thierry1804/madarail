...
interface MenuItem {
  ...
  /** Si défini, la vue n'est visible que pour les rôles listés */
  roles?: Array<User['role']>;
}

const menuItems: MenuItem[] = [
  { id: 'dashboard',   label: 'Tableau de bord',     icon: LayoutDashboard, roles: ['admin'] },
  { id: 'pos',         label: 'Vente de billets',     icon: Ticket,          roles: ['admin', 'agent'] },
  { id: 'reservation', label: 'Réservations',         icon: BookOpen,        roles: ['admin', 'agent'] },
  { id: 'routes',      label: 'Trajets',              icon: Train,           roles: ['admin', 'agent'] },
  { id: 'sales',       label: 'Historique',           icon: History },
  { id: 'bluetooth',   label: 'Sync Bluetooth',       icon: Bluetooth,       roles: ['agent', 'controller'] },
...
export function Sidebar({
  ...
  const { currentView, setCurrentView, currentUser, setCurrentUser, isOnline } = useApp();
  ...
  const filteredMenuItems = menuItems.filter(
    item => !item.roles || item.roles.includes(currentUser?.role as User['role'])
  );
  ...
  return (
    <aside
      className={`
        ...
        w-[min(18rem,85vw)] pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)]
        lg:static lg:pt-0 lg:pb-0 lg:shadow-none lg:shrink-0
    ...
    >
      ...
      <div className={`p-4 border-b border-madarail-navy-bright ${collapsed ? 'lg:p-3' : ''}`}>
        ...
        <p className={`text-[11px] text-slate-400 mt-3 uppercase tracking-wide ${collapsed ? 'lg:hidden' : ''}`}>
          Madagascar
        </p>
      </div>
        ...
        {filteredMenuItems.map(item => {
          ...
          return (
            ...
            >
              ...
              <span className={`font-medium text-left ${collapsed ? 'lg:hidden' : ''}`}>
                {item.label}
              </span>
            </button>
          );
      ...
      <div className={`p-4 border-t border-madarail-navy-bright ${collapsed ? 'lg:p-3' : 'lg:p-6'}`}>
        {/* Badge hors-ligne */}
        {!isOnline && (
          <div className={`mb-3 flex items-center gap-2 rounded-lg bg-amber-500/20 px-3 py-2 text-amber-300 text-xs font-semibold ${collapsed ? 'lg:justify-center' : ''}`}>
            <WifiOff className="w-4 h-4 shrink-0" />
            <span className={collapsed ? 'lg:hidden' : ''}>Hors ligne</span>
          </div>
        )}
        ...
        <div className={`mb-4 ${collapsed ? 'lg:hidden' : ''}`}>
          ...
          <p className="text-xs text-slate-500 truncate">{currentUser?.gare}</p>
        </div>
        <button
          type="button"
